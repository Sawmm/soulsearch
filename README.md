...
- **Ghostty Optimised:** Custom Dracula-inspired theme designed for readability on dark backgrounds.
...
4. **Link the command globally:**
   ...
   npm link
   ```
   You can now run it anywhere with `soulsearch`.
...
| Setting | Description |
|---|---|
| `portForwarded` | Set to `false` (Restricted Mode) if you haven't forwarded port **2234**. The app will only show results with open slots to ensure downloads start. |
| `autoConvert.smartMode` | High-quality sources become AIFF (CDJ compatible); low-quality sources become MP3. |
| `autoConvert.detectFakeBitrate` | Uses the **FakeFLAC method** to detect upscaled fake lossless files. The file is re-encoded through 320k MP3 and back, then both versions are spectrally analysed above 14kHz. If the frequency ceilings match, the source was already lossy. The actual quality is shown in the Downloads `CONVERSION` column. |
| `autoConvert.normalizeVolume` | Runs files through FFmpeg's `loudnorm` filter to equalise to `targetLufs` (default `-14.0`). Useful for DJs who need consistent perceived volume across tracks. |
| `autoConvert.smartFolders` | Parses ID3 metadata and sorts finished files into a `Downloads/Genre/Artist/` hierarchy. |
...
| `Spacebar` | Stream/play a downloaded track (requires `ffplay` or macOS `afplay`) |
| `x` | Cancel selected active download or conversion |
| `c` | Clear finished/errored downloads from the list |
